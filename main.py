...
def get_db_path():
    if platform == "android":
        ...
        return os.path.join(app_storage_path(), "students.db")
    return "students.db"
...
def get_all_payments():
    conn = sqlite3.connect(get_db_path())
    cursor = conn.cursor()
    cursor.execute("""SELECT p.payment_id, p.student_id, s.name, s.aadhaar, p.amount_paid, p.payment_date
...
def export_data():
    try:
        ...
        student_file = get_export_path("students.csv")
        ...
        payment_file = get_export_path("payments.csv")
        with open(payment_file, "w", newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(payment_columns)
            writer.writerows(payments)

        conn.close()
        return f"Exported to {student_file} and {payment_file}"
    except Exception as e:
...
class StudentApp(MDApp):
    def build(self):
        ...
        create_tables()
        return Builder.load_string(KV)  # use the full KV string you already pasted
